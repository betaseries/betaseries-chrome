...
BS = {
  ...
  membersEpisodes: function(lang) {
    ...
    return {
      ...
      root: 'episodes',
      content: function(data) {
        var classes, date_0, dlSrtLanguage, downloaded, empty, episode, extraEpisodes, extraIcon, extraText, extra_episodes, hidden, hiddenShow, hidden_shows, imgDownloaded, jours, n, nbSubs, nbrEpisodes, nbrEpisodesPerSerie, newShow, newTitleShow, output, posEpisode, quality, remain, season, show, stats, sub, subs, textTitle, texte2, texte3, time, title, titleIcon, url, visibleIcon, _ref, _ref1;
        output = "";
        show = "";
        nbrEpisodes = 0;
        posEpisode = 1;
        ...
        for (n in data) {
          if (newTitleShow) {
            ...
            extraEpisodes = (_ref1 = data[n].url, __indexOf.call(extra_episodes, _ref1) >= 0);
            if (hiddenShow) {
      ...
      content: function() {
        ...
        $.ajax({
          ...
          success: function(data) {
            var desc, i, item, items, link, linkOrig, title, titleOrig, _i, _ref, _results;
            items = $(data).find('item');
            _results = [];
            for (i = _i = 0, _ref = Math.min(5, items.length); 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
              item = $(items[i]);
              titleOrig = item.find('title').text();
              title = titleOrig.substring(0, 40);
              if (titleOrig.length > 40) {
                title += '..';
              }
